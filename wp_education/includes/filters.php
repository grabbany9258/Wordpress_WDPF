...
class WPLMS_Filters{
    ...
    */
	...
	function check_attachments($args,$post,$user_id){
    	if(function_exists('vibebp_get_setting') && vibebp_get_setting('instructor_privacy')){
    		if(!empty($user_id)){
    		...
    		$instructor_privacy = vibe_get_option('instructor_content_privacy');
	    	if(isset($instructor_privacy) && $instructor_privacy && !user_can($user_id,'manage_options' )){
	    		if(!empty($user_id)){
			        $args['author'] = intval($user_id);
		    	}
	    	}
	...
	function vibe_registration_redirect() {
    	...
    	if($pageid == 'dashboard'){
    		...
    		if(defined('WPLMS_DASHBOARD_SLUG')){
    			$link = bp_loggedin_user_domain().WPLMS_DASHBOARD_SLUG.'?vibebp_get_setting='.function_exists('vibebp_get_setting').'@'.vibebp_get_setting('bp_single_page');
